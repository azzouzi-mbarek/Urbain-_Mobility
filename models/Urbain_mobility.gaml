/***
* Name: StoryTellingModel
* Author: AZZOUZI M'barek
* 
* 
* 
* Description: 
Un modèle multi-niveaux basé sur des agents simulant (people, bus, batiment....)
 le flux de trafic  développé ,
 Il modélise les comportements des agents dans différents mode de deplacements qautediant 
 circulant dans une ville. Il montre comment la mobilité peut changer l'ambiance locale de la ville.
  Le modèle et la sortie de simulation permettent de voir l'impact de différents modes de mobilité sur le flux de trafic et la congestion.
***/


model Urbainmobility

global {
	/*
 * Données SIG:
 *Une description SIG du plan d'urbanisme proposé composé de bâtiments et de routes. Usages de 
 chaque bâtiment (types d'habitations, types de bureaux, écoles, équipements culturels, types de parcs, etc.).
 */
	...
	//ENVIRONMENT
	float step <- 1 #mn;
	date starting_date <-date([2020,1,20,0,0]);
	string case_study <- "data";
	int nb_people <- 500;

    string Folder <- "./../includes/City/"+case_study;
	file<geometry> buildings_shapefile <- file<geometry>(Folder+"/Buildings.shp");
	file<geometry> roads_shapefile <- file<geometry>(Folder+"/Roads.shp");
	geometry shape <- envelope(roads_shapefile);
	
 /*
  *Données de mobilité
Définir les types de modes de mobilité pour le projet, y compris les caractéristiques des véhicules
 des modes existants et nouveaux (# passagers, vitesse, autonome vs conventionnel, etc.). Définissez des hypothèses
 sur les modèles d'utilisation de la mobilité des personnes (agents) qui seront utilisés pour établir
  les paires origine / destination des comportements des agents.
  */
  	list<string> mobility_list <- ["walking", "bike","car","bus"];
	file activity_file <- file("./../includes/MobilityData/ActivityPerProfile.csv");
	file criteria_file <- file("./../includes/MobilityData/CriteriaFile.csv");
	file profile_file <- file("./../includes/MobilityData/Profiles.csv");
	file mode_file <- file("./../includes/MobilityData/Modes.csv");
	file weather_coeff <- file("../includes/MobilityData/weather_coeff_per_month.csv");
	...
	/*
 Modèles de navettage
Faire des hypothèses sur les comportements de navettage: en particulier, combien de personnes vivraient
 et travailleraient dans le district et celles qui font la navette chaque jour depuis l'extérieur du district. Des hypothèses
  seront également formulées concernant les comportements à l'égard des personnes qui entrent et sortent du quartier
  pour d'autres activités sans travail (loisirs, shopping, etc.)

Profils d'agent
Définissez les profils des personnes, des nombres et des points d'origine et de destination
 tels que déterminés par les lieux de vie, de travail et d'autres activités - y compris les personnes
  qui vivent dans le quartier et celles qui font la navette. Profils de personnes vivant dans chaque
  type de bâtiment résidentiel, travaillant dans chaque type de bâtiment commercial, et attirées vers la
  3e place (étudiants, jeunes professionnels, familles, travailleurs à mi-carrière, cadres supérieurs, etc.). 
  Ces informations seront utilisées pour établir les comportements de chaque type d'agent: où ils vivent,
 */
...
species building {
	...
	string category;
	rgb color <- #grey;
	float height <- rnd(20.0,30.0);//50.0 + rnd(50);
	aspect default {
...
experiment Mobility type: gui {
	output {
		display map type: opengl draw_env: false background: #black refresh:every(10#cycle){
            ...
            chart "% by tronsport modet" background:#black type: pie style:ring size: {0.5,0.5} position: {world.shape.width*1.1,world.shape.height*0} color: #white axes: #yellow title_font: 'Helvetica' title_font_size: 12.0 
